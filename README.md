...
**Latest sweep — 2026-07-12** · 20/45 projects clean (zero critical/high)

| Provider | 🚨 critical | ⚠️ high | 🟡 manual-review | Rules that fired |
|---|---:|---:|---:|---|
| n8n integrations | 82 | 28 | 0 | `n8n/missing-signature-verification` (×52)<br>`n8n/missing-timestamp-check` (×28)<br>`n8n/missing-timing-safe-equal` (×26)<br>`n8n/raw-body-misuse` (×4) |
...
_Coverage note: the engine couldn't parse **237** files across the corpus (broken syntax or language features the parser doesn't model). Those are scan-coverage diagnostics — not handler bugs — and are excluded from the table above._
