...
import { readFileSync, mkdtempSync, readdirSync, statSync, mkdirSync } from 'node:fs';
...
function checkPackageJson(pkgPath: string): Violation[] {
  ...
  for (const name of FORBIDDEN_SCRIPTS) {
    if (pkg.scripts?.[name]) out.push({ file: 'package.json', category: 'lifecycle-script', pattern: name });
  }
  return out;
}
...
async function main(): Promise<number> {
  ...
  try {
    ...
    if (tgzs.length !== 1) throw new Error(`expected 1 .tgz in ${tmp}, found ${tgzs.length}`);
    const extractDir = join(tmp, 'extracted');
    mkdirSync(extractDir, { recursive: true });
    await extract({ file: join(tmp, tgzs[0]!), cwd: extractDir });
    ...
    try { execSync(`rm -rf "${tmp}"`); } catch { /* best effort */ }
  }
  ...
  console.log(pc.green('[OK] Bundle inspection passed.'));
  return 0;
}
