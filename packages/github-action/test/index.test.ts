...
beforeEach(async () => {
  ...
  sampleScan = JSON.parse(
    await fs.readFile(
      path.join(__dirname, 'fixtures/sample-scan.json'),
      'utf8',
    ),
  ) as ScanJsonEnvelope;
...
function setExecToReturnEnvelope(envelopes: ScanJsonEnvelope[]): void {
  ...
  mocks.exec.mockImplementation(
    (cmd: string, args: ReadonlyArray<string>, opts: ExecOptions = {}) => {
      const isHookwardenScan =
        cmd === 'node' && args.includes('scan');
      if (!isHookwardenScan) {
...
function envelopeWithExtraFinding(
  base: ScanJsonEnvelope,
  hash: string,
): ScanJsonEnvelope {
...
describe('ACTION-03 SARIF upload', () => {
  ...
  it('skips SARIF upload + PR comment on fork PR (T-05-04-01 / T-05-03-04)', async () => {
    ...
    expect(mocks.createComment).not.toHaveBeenCalled();
    const fork = mocks.warning.mock.calls.flat().some((m) =>
      typeof m === 'string' && m.toLowerCase().includes('fork'),
    );
...
describe('Output setting (D-88 — all 4 outputs)', () => {
  it('sets findings-count, new-findings-count, findings-by-severity, sarif-path', async () => {
    ...
    const findingsCountCall = mocks.setOutput.mock.calls.find(
      (c) => c[0] === 'findings-count',
    );
    expect(findingsCountCall?.[1]).toBe(6);
    // non-PR event → new-findings-count = 0
    const newFindingsCall = mocks.setOutput.mock.calls.find(
      (c) => c[0] === 'new-findings-count',
    );
