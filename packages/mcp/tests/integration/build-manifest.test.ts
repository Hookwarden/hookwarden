...
import { RULES_PACK_VERSION } from "@hookwarden/rules";
...
describe("dist/build-manifest.json — shape contract (Test 1)", () => {
  ...
  // Versions are sourced from the live ENGINE_VERSION / RULES_PACK_VERSION
  // constants (generated by per-package scripts/sync-version.mjs from each
  // package.json#version on every install). Hard-coded "0.7.4"-style
  // expectations made every Changesets-driven release fail preflight and
  // forced manual test edits — the test isn't validating a version literal,
  // it's validating that the manifest emitter reads from the same source the
  // engine itself exposes.
  it("declares engine.version === ENGINE_VERSION with content_hash null", () => {
    expect(manifest.engine.version).toBe(ENGINE_VERSION);
    expect(manifest.engine.content_hash).toBeNull();
  });

  it("declares rules.version === RULES_PACK_VERSION with a 64-char SHA-256 hex content_hash", () => {
    expect(manifest.rules.version).toBe(RULES_PACK_VERSION);
